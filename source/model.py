...
def solve_with_emission_boundary_check(y_grid, model_paras=()):
    η, δ, θ, πc_o, σ_y, γ_1, γ_2, ϕ_i, πd_o, e_tilde, h, πc, bc, ξ_w, ξ_p, ξ_a = model_paras
    Δ_y = y_grid[1] - y_grid[0]
    ...
    A = np.ones_like(y_grid)*(-δ)
    B = (np.sum(πc*θ, axis=0) + σ_y*h)*e_tilde
    C = .5*σ_y**2*e_tilde**2
    D = δ*η*np.log(e_tilde) + (η-1)*d_Λ*e_tilde*(np.sum(πc*θ, axis=0)+σ_y*h) + .5*(η-1)*dd_Λ*σ_y**2*e_tilde**2\
        + ξ_a*c_entropy + ξ_w/2.*h**2
...
#     v0 = solve_lienar_ode_python(A, B, C, D, 0., Δ_y, (0, bc), (False, True))
    ...
    while error > 1e-8 and count < 10_000:
        v0 = false_transient_one_iteration_python(A, B, C, D, v0, .3, Δ_y, (0, bc), (False, True))
        v0_dy = compute_derivatives(v0, 1, Δ_y, central_diff=False)
        v0_dyy = compute_derivatives(v0, 2, Δ_y)
        rhs_error = A*v0 + B*v0_dy + C*v0_dyy + D
        rhs_error = np.max(abs(rhs_error))
        lhs_error = np.max(abs((v0 - v_old)/.3))
        error = lhs_error
        count += 1
        v_old = v0.copy()
        print("Iteration %s: LHS Error: %s; RHS Error %s" % (count, lhs_error, rhs_error))        
...
def uncertainty_decomposition(y_grid, model_paras=(), e_tilde=None, h=None, πc=None, bc=None,
    ...
    while error > tol and count < max_iter:
        ...
        C = .5 * σ_y**2 * e_tilde**2
        D = δ*η*np.log(e_tilde) + (η-1)*d_Λ*e_tilde*(np.sum(πc*θ, axis=0)+σ_y*h)\
            + .5*(η-1)*dd_Λ*σ_y**2*e_tilde**2 + ξ_w/2.*h**2\
            + ξ_a*c_entropy
